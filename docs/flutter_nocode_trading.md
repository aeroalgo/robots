...
- **Как работает**:
  ...
  - Каждый шаг содержит выражение на иDSL: `if crossOver(EMA(close, 20), EMA(close, 50)) then BUY qty=10`.
...
- **Пример сценария**:
  - Ввод формулы: `signal = SMA(close, 14) - SMA(close, 28)`; затем `if signal > 0 and volume > avgVolume(20) then BUYw`.
- **Плюсы**:
