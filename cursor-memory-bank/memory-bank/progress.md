...
## 2025-11-13 — Strategy Layer базовый каркас
- Создан модуль `src/strategy` (base, types, builder, context, registry) для описания стратегий
- Реализованы traits `Strategy`, `StrategyDescriptor`, динамическая стратегия и реестр стратегий
- Добавлен `StrategyContext` с поддержкой мульти-таймфреймов, индикаторов и пользовательских параметров
- Актуализирован `tasks.md`: задача по базовой системе стратегий отмечена как выполняющаяся
- Выполнен `cargo check` — сборка успешна

## 2025-11-13 — Strategy Layer стоп-обработчики
- Интегрированы обработчики StopLossPct и TakeProfitPct на базе vvTS, формирующие приоритетные `Exit`-сигналы до правил стратегии
- Расширен `StrategyContext`: добавлено хранение активной позиции и методы управления ей
- `StrategyDecision` включает коллекцию `stop_signals` и сохраняет цену выхода в `metadata`
- Добавлены юнит-тесты `stop_handlers_trigger_exit_signal` для валидации работы стопов

## 2025-11-13 — Планирование управляющих компонентов Strategy Layer
- Сформирован план разработки менеджера позиций: сущности `PositionState`/`OrderTicket`, хранилище активных позиций, обработка сигналов `StrategyDecision`, события и тестовые сценарии
- Запланирован исполняющий слой: интерфейс `StrategyExecutor`, цикл обновления `StrategyContext` → `evaluate`, интеграция с менеджером позиций и сбор метрик
- Обновлён `tasks.md`: соответствующие задачи помечены как «Планирование» с детальным чек-листом шагов

## 2025-11-13 — Position Manager первая версия
- Реализован модуль `src/strategy/positions.rs` с сущностями `PositionState`, `OrderTicket`, `PortfolioState`, событиями и менеджером `PositionManager`
- Обновлён `StrategyContext`: добавлены активные позиции, хранение символа таймфрейма и методы синхронизации с менеджером
- Расширен `ActivePosition` дополнительными полями (id, символ, таймфрейм, последняя цена, PnL) для корректной работы стопов и контекста
- Написаны юнит-тесты `opens_and_closes_position` и `stop_signal_closes_position_first`, покрывающие открытие/закрытие позиций и приоритет стоп-сигналов
- `cargo test` завершается ошибками из-за отсутствующего `ConditionInput` в модуле `condition`; изменения менеджера компилируются корректно при устранении зависимой проблемы

## 2025-11-13 — Strategy Executor бэктест
- Создан модуль `src/strategy/executor.rs` с `BacktestExecutor`, управлением `StrategyContext`, `PositionManager` и обработкой сигналов стратегии
- Реализован `HistoricalFeed` для пошагового обновления таймфреймов, синхронизации контекста и поддержки нескольких временных рамок
- Добавлены структуры `StrategyTrade`, `BacktestMetrics`, `BacktestReport` с фиксацией сделок, equity curve и базовых метрик (PnL, win rate, средний результат)
- Написан юнит-тест `backtest_executor_produces_trade_and_metrics`, подтверждающий цепочку evaluate → position manager → сделки/метрики

## 2025-11-15 — Multi-entry позиции и риск-менеджмент
- `PositionKey`, `PositionState`, `ActivePosition`, `ClosedTrade` и `StrategyTrade` расширены полем `entry_rule_id`, что позволило вести отдельные позиции на один инструмент по различным правилам входа
- `PositionManager` обновлён: ключи открытых позиций включают `entry_rule_id`, закрытия и стопы сопоставляют `target_entry_ids` как с группами, так и с конкретными правилами, а журналы сделок сохраняют `entry_rule_id`/`exit_rule_id`
- `BacktestReport` и вывод в `main.rs` теперь отображают правило, открывшее и закрывшее сделку, что улучшает отладку и анализ стратегий
- Задача "Реализация риск-менеджмента" в `tasks.md` отмечена выполненной, прогресс по Strategy Layer увеличен до 2/6 задач (33%)
