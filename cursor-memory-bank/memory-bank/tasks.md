...
### 🆕 Запланировано
- [ ] **Formula Gateway для пользовательских индикаторов**
  ...
  - **Шаги**:
    1. Спроектировать DSL/AST и список допустимых операций (арифметика, логика, агрегаты) с проверкой типов.
    2. Расширить `IndicatorFactory` поддержкой вычислительного движка и кеширования результатов на таймфреймах.
    3. Добавить API/метаданные для сохранения пользовательских формул и маппинга их на `strategy::presets`.
  - **Зависимости**: Indicator Layer, Strategy Presets.

- [ ] **Formula Gateway: арифметический слой**
  ...
  - **Шаги**:
    1. Спроектировать DSL/AST для арифметики (операции сложения/вычитания/умножения, деления, агрегаты) с проверкой типов и контролем таймфреймов.
    2. Расширить `IndicatorFactory` вычислительным движком и кешированием результатов, чтобы формульные индикаторы выглядели как обычные bindings.
    3. Подготовить декларативный формат описания формул (JSON/Graph схема) для интеграции с Flutter NoCode UI.
  - **Зависимости**: Indicator Layer, Strategy Presets.
...
- [ ] **Formula Gateway: логический слой условий**
  ...
  - **Шаги**:
    1. Ограничить `ConditionBindingSpec` допустимыми операциями (>, <, crosses, between, выражения на готовых числовых сериях).
    2. Добавить декларативное описание условий (списки индикаторов + оператор) для сериализации из NoCode/Fake DSL.
    3. На следующем этапе (позже) дополнить API/метаданные сохранением пользовательских условий, когда арифметический слой стабилизирован.
