...
use crate::data_access::{
    ...
    models::*,
    query_builder::{CacheQueryBuilder, CounterQueryBuilder, KeyUtils, QueueQueryBuilder},
    Cache, DataSource,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(signal) = cached_signal {
        println!(
            "📈 Получен сигнал: {:?} с уверенностью {:.2}",
    ...
    println!(
        "📋 Cache Query Builder создан с {} операциями",
        cache_query.operations().len()
    );
    ...
    println!(
        "📋 Queue Query Builder создан с {} операциями",
        queue_query.operations().len()
    );
    ...
    println!(
        "📋 Counter Query Builder создан с {} операциями",
        counter_query.operations().len()
    );
