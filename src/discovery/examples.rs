...
pub fn example_strategy_generation() {
    ...
    let config = StrategyDiscoveryConfig {
        ...
        global_param_ranges: HashMap::new(),
    ...
    let candidates = engine.generate_strategies(
        &indicators,
        &price_fields,
        &operators,
    );
    ...
    for (i, candidate) in candidates.iter().take(5).enumerate() {
        let ind_names: Vec<&str> = candidate.indicators.iter().map(|ind| ind.name.as_str()).collect();
        let cond_names: Vec<&str> = candidate.conditions.iter().map(|cond| cond.name.as_str()).collect();
        let tf_strs: Vec<String> = candidate.timeframes.iter().map(|tf| format!("{:?}", tf)).collect();
        
        println!("   Кандидат {}:", i + 1);
        println!("      Индикаторы: {:?}", ind_names);
        println!("      Условия: {:?}", cond_names);
        println!("      Таймфреймы: {:?}", tf_strs);
        println!("      Параметров оптимизации: {}", candidate.total_optimization_params());
        println!();
    }
