...
impl ATRTrailIndicatorStopHandler {
    pub fn new(
        period: f64,
        coeff_atr: f64,
    ...
    fn get_atr_value_for_validation(&self, ctx: &StopValidationContext<'_>) -> Option<f32> {
        let aux_alias = self.atr_auxiliary_alias();
        if let Some(value) = ctx.timeframe_data.auxiliary_value_at(&aux_alias, ctx.index) {
            return Some(value);
        }
...
impl StopHandler for ATRTrailIndicatorStopHandler {
    fn name(&self) -> &str {
    ...
    fn required_auxiliary_indicators(&self) -> Vec<AuxiliaryIndicatorSpec> {
        ...
        specs.push(AuxiliaryIndicatorSpec::atr(self.period as u32));
        ...
        specs.push(AuxiliaryIndicatorSpec {
