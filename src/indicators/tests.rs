...
mod tests {
    ...
    async fn test_bollinger_bands_components() {
        ...
        // Тестируем BBMiddle
        let bb_middle = BBMiddle::new(10.0, 2.0).unwrap();
        let middle_values = bb_middle.calculate_ohlc(&ohlc_data).unwrap();

        assert_eq!(middle_values.len(), ohlc_data.len());
        assert!(middle_values[9] > 0.0); // Первое значение SMA

        // Тестируем BBUpper
        let bb_upper = BBUpper::new(10.0, 2.0).unwrap();
        let upper_values = bb_upper.calculate_ohlc(&ohlc_data).unwrap();

        assert_eq!(upper_values.len(), ohlc_data.len());
        assert!(upper_values[9] > middle_values[9]); // Верхняя линия должна быть выше средней

        // Тестируем BBLower
        let bb_lower = BBLower::new(10.0, 2.0).unwrap();
        let lower_values = bb_lower.calculate_ohlc(&ohlc_data).unwrap();

        assert_eq!(lower_values.len(), ohlc_data.len());
        assert!(lower_values[9] < middle_values[9]); // Нижняя линия должна быть ниже средней
    }
    ...
    async fn test_keltner_channel_components() {
        ...
        // Тестируем KCMiddle
        let kc_middle = KCMiddle::new(10.0).unwrap();
        let middle_values = kc_middle.calculate_ohlc(&ohlc_data).unwrap();

        assert_eq!(middle_values.len(), ohlc_data.len());
        assert!(middle_values[9] > 0.0); // EMA должна быть положительной после прогрева периода

        // Тестируем KCUpper
        let kc_upper = KCUpper::new(10.0, 10.0, 2.0).unwrap();
        let upper_values = kc_upper.calculate_ohlc(&ohlc_data).unwrap();

        assert_eq!(upper_values.len(), ohlc_data.len());
        assert!(upper_values[9] > middle_values[9]); // Верхняя линия должна быть выше средней после прогрева

        // Тестируем KCLower
        let kc_lower = KCLower::new(10.0, 10.0, 2.0).unwrap();
        let lower_values = kc_lower.calculate_ohlc(&ohlc_data).unwrap();

        assert_eq!(lower_values.len(), ohlc_data.len());
        assert!(lower_values[9] < middle_values[9]); // Нижняя линия должна быть ниже средней после прогрева
    }
