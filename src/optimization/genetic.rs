...
use crate::optimization::types::{EvaluatedStrategy, GeneticAlgorithmConfig, GeneticIndividual, Population};
...
impl GeneticAlgorithm {
    ...
    pub async fn evolve_generation(
        ...
        while new_individuals.len() < population.individuals.len() - elites.len() {
            ...
            if let Some((child1_params, child2_params)) = self.population_manager.crossover(
                parents[0],
                parents[1],
            ) {
                ...
                if let Some(ref candidate) = parents[0].strategy.candidate {
                    self.population_manager.mutate(&mut child1_params, candidate);
                }
                if let Some(ref candidate) = parents[1].strategy.candidate {
                    self.population_manager.mutate(&mut child2_params, candidate);
                }

                if let Some(ref candidate) = parents[0].strategy.candidate {
                    let child1 = self.create_individual(
                        candidate.clone(),
                        child1_params,
                ...
                if new_individuals.len() < population.individuals.len() - elites.len() {
                    if let Some(ref candidate) = parents[1].strategy.candidate {
                        let child2 = self.create_individual(
                            candidate.clone(),
                            child2_params,
                            population.generation + 1,
                            population.island_id,
    ...
    async fn create_individual(
        ...
        let report = self.evaluator.evaluate_strategy(&candidate, parameters.clone()).await?;
