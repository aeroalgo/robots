...
pub trait MomentumCondition: Condition {
    /// Проверить ускорение
    async fn is_accelerating(&self, data: &[f32], period: usize) -> bool;

    /// Проверить замедление
    async fn is_decelerating(&self, data: &[f32], period: usize) -> bool;

    /// Проверить разворот
    async fn is_reversing(&self, data: &[f32], period: usize) -> bool;

    /// Получить моментум
    async fn get_momentum(&self, data: &[f32], period: usize) -> ConditionResult<f32>;
}
...
pub trait VolatilityCondition: Condition {
    ...
    async fn is_high_volatility(&self, data: &[f32], period: usize, threshold: f32) -> bool;

    /// Проверить низкую волатильность
    async fn is_low_volatility(&self, data: &[f32], period: usize, threshold: f32) -> bool;

    /// Получить уровень волатильности
    async fn get_volatility_level(&self, data: &[f32], period: usize) -> ConditionResult<f32>;

    /// Проверить расширение волатильности
    async fn is_expanding(&self, data: &[f32], period: usize) -> bool;

    /// Проверить сжатие волатильности
    async fn is_contracting(&self, data: &[f32], period: usize) -> bool;
}
...
pub trait ParameterOptimizer {
    ...
    fn get_parameter_ranges(&self) -> HashMap<String, (f32, f32, f32)>;
    ...
    async fn optimize_parameters(
        ...
        data: &[f32],
        target_metric: &str,
    ) -> ConditionResult<HashMap<String, f32>>;
